...
async function resolvePageId(slug, client = pool) {
    const { rows } = await client.query(`SELECT id FROM pages WHERE slug = $1 LIMIT 1`, [slug]);
...
async function loadPublishedPageBySlug(slug) {
...
router.get('/published', async (req, res) => {
    ...
    try {
      ...
      `, params);
        ...
        const pages = limitedRows.map((row) => PageSummarySchema.parse({
