...
import { RecipePageSchema, RecipePageSummarySchema, RecipePageUpdateSchema, RecipePageCreateSchema, } from 'bie-models';
...
function mapVersionToRecipePage(row) {
...
async function resolveRecipePageId(slug, client = pool) {
    const { rows } = await client.query(`SELECT id FROM recipe_pages WHERE slug = $1 LIMIT 1`, [slug]);
    return rows[0]?.id ?? null;
}
async function loadRecipePageById(pageId, client = pool) {
...
async function loadRecipePageBySlug(slug) {
...
async function loadPublishedRecipePageBySlug(slug) {
...
router.get('/published', async (req, res) => {
    ...
    try {
      ...
      `, params);
        ...
        const pages = limitedRows.map((row) => RecipePageSummarySchema.parse({
            page: mapVersionToRecipePage(row),
        }));
        let nextCursor = null;
        if (hasMore && limitedRows.length) {
            const lastRow = limitedRows[limitedRows.length - 1];
            const cursorDate = lastRow.published_sort ?? lastRow.published_at ?? lastRow.version_updated_at ?? lastRow.page_updated_at ?? lastRow.updated_at;
            if (cursorDate) {
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        `, [pageId, status, payload.title, recipeJson, metaJson, createdBy, publishedAt]);
            ...
            if (status === 'published') {
                await client.query(`UPDATE recipe_versions SET status = 'draft' WHERE page_id = $1 AND id <> $2 AND status = 'published'`, [pageId, version.id]);
            }
            await client.query(`
...
router.put('/:slug', async (req, res) => {
    ...
    try {
        const pageId = await withTransaction(async (client) => {
            ...
            const { rows: nextVersionRows } = await client.query(`SELECT COALESCE(MAX(version), 0) + 1 AS next_version FROM recipe_versions WHERE page_id = $1`, [resolvedPageId]);
            const nextVersion = Number(nextVersionRows[0]?.next_version ?? 1);
            if (status === 'published') {
                await client.query(`UPDATE recipe_versions SET status = 'draft' WHERE page_id = $1 AND status = 'published'`, [resolvedPageId]);
            }
            const versionInsert = await client.query(`
        INSERT INTO recipe_versions (page_id, version, status, title, recipe, meta, created_by, published_at)
...
router.post('/:slug/versions/:versionId/publish', async (req, res) => {
    ...
    try {
        const pageId = await withTransaction(async (client) => {
            ...
            const { rows: versionRows } = await client.query(`SELECT id FROM recipe_versions WHERE id = $1 AND page_id = $2 LIMIT 1`, [req.params.versionId, resolvedPageId]);
            if (!versionRows.length) {
                throw new Error('VERSION_NOT_FOUND');
            }
            await client.query(`UPDATE recipe_versions SET status = 'draft' WHERE page_id = $1 AND id <> $2 AND status = 'published'`, [resolvedPageId, req.params.versionId]);
            await client.query(`
        ...
        `, [resolvedPageId, req.params.versionId, publishedAt]);
            await client.query(`UPDATE recipe_pages SET published_version_id = $1, updated_at = now() WHERE id = $2`, [req.params.versionId, resolvedPageId]);
            return resolvedPageId;
        });
        const page = await loadRecipePageById(pageId);
        if (!page) {
